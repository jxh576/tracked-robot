...
ScanDataBuilder::ScanDataBuilder() :
    data(), method_index(0) {
    sprintf(irrelevant_data, "0 0 0 0 0 0%c", lms100_cola::ETX_BYTE);
    irrelevant_data_len = strlen(irrelevant_data);
}

ScanDataBuilder::ScanDataBuilder(size_t expected_data_count) :
    data(expected_data_count), method_index(0) {
    sprintf(irrelevant_data, "0 0 0 0 0 0%c", lms100_cola::ETX_BYTE);
    irrelevant_data_len = strlen(irrelevant_data);
}
...
bool ScanDataBuilder::get_next_string(char*& src, size_t& src_size, char* dest,
    ...
    if (((size_t) token_length) >= dest_size) {
        //ROS_FATAL("unexpected message format \"%s\"", src);
    }
...
int main() {
    ...
    builder.Parse(temp_packet, size);
    ScanData data = builder.GetData();
    std::cout << "scan frequency: " << data.scan_frequency << std::endl;
    std::cout << "measurement frequency: " << data.measurement_frequency << std::endl;
    std::cout << "scale factor: " << data.scale_factor << std::endl;
    std::cout << "scale factor offset: " << data.scale_factor_offset << std::endl;
